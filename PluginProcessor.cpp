...
//==============================================================================
const String DirtyboyAudioProcessor::getName() const
{
...
const String DirtyboyAudioProcessor::getProgramName (int index)
{
    return {};
}

void DirtyboyAudioProcessor::changeProgramName (int index, const String& newName)
{
}
...
void DirtyboyAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (int audioSample = 0; audioSample < buffer.getNumSamples(); audioSample++) {
            ...
            const float cleanSignal = channelData[audioSample]*10.0f; //could also try 6.0
            
            float destroyedii = (std::atan(cleanSignal) / std::atan(abs(cleanSignal))) * (1.0f - std::exp(std::pow(std::atan(cleanSignal), 2.0f) * (std::atan(std::abs(cleanSignal)))))
            * ((cleanSignal * (2.0f)) / 2.0f) * (DirtyMix / 30.0f);
            
            
            channelData[audioSample] = destroyedii + (cleanSignal);
            
            // code which impliments a limiter within the system
            /* if (destroyedii > 1.0){
                destroyedii = jmax(destroyedii, 1.0f); */
...
AudioProcessorEditor* DirtyboyAudioProcessor::createEditor()
{
